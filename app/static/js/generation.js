...
function exportFiles(url, defaultFileName, isProfessor = false, isFreeRoom = false) {
    ...
    // ✨ إضافة السطر الخاص بجداول القاعات والمدرجات ✨
    else if (url.includes('rooms-schedule')) {
        finalFileName = lang === 'en' ? 'Schedules_Rooms.xlsx' : (lang === 'fr' ? 'Emplois_Salles.xlsx' : 'جداول_القاعات.xlsx');
    }
...
function importExcelSchedule() {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            // ✨ السطر الذي اكتشفته: تحديث القاعات الفارغة
...
function importComprehensiveSchedule() {
    ...
    if (!fileInput || !fileInput.files.length) {
        return alert("الرجاء اختيار ملف 'القائمة الشاملة' أولاً!");
    }
    ...
    const btn = event.target;
    const originalText = btn.innerHTML;
    btn.innerHTML = "⏳ جاري الهندسة العكسية وبناء الجداول...";
    btn.disabled = true;

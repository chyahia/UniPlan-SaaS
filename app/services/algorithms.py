...
# 🗑️ تم حذف مكتبة threading و state_lock و SCHEDULING_STATE
# في بيئة SaaS، نعتمد على عزل العمليات. جميع بيانات الحالة تُمرر عبر scheduling_state 
# وجميع السجلات تُرسل عبر log_q إلى Redis.
...
def run_large_neighborhood_search(log_q, all_lectures, days, slots, rooms_data, teachers, all_levels, identifiers_by_level, special_constraints, teacher_constraints, distribution_rule_type, lectures_by_teacher_map, globally_unavailable_slots, saturday_teachers, teacher_pairs, day_to_idx, rules_grid, max_iterations, ruin_factor, prioritize_primary, scheduling_state, last_slot_restrictions, level_specific_large_rooms, specific_small_room_assignments, constraint_severities, initial_solution=None, max_sessions_per_day=None, consecutive_large_hall_rule="none", progress_channel=None, prefer_morning_slots=False, use_strict_hierarchy=False, non_sharing_teacher_pairs=[], mutation_hard_intensity=3, mutation_soft_probability=0.5, lns_stagnation_threshold=100):
    ...
    unplaced, hard, soft_sum, soft_count = -initial_fitness[0], -initial_fitness[1], -initial_fitness[2], -initial_fitness[3]
    log_q.put(f'   - اللياقة الأولية (نقص, صارم, مرن) = ({unplaced}, {hard}, {soft_count}={soft_sum})')
    time.sleep(0)
    ...
    # 🔧 تصحيح وتفعيل حساب حد الركود ديناميكياً بناءً على مدخلات المستخدم من الواجهة
    thresh_val = float(lns_stagnation_threshold)
    actual_threshold = 15.0 if thresh_val >= 100.0 or thresh_val <= 0 else thresh_val
    stagnation_percentage = actual_threshold / 100.0
    ...
    for i in range(max_iterations):
        ...
        # ✨✨ --- بداية الجزء الخاص بالطفرة اليدوية --- ✨✨
        if scheduling_state.get('force_mutation'):
            ...
            if 'mutation_intensity' in scheduling_state:
        ...
        if best_fitness_so_far == (0, 0, 0, 0):
            log_q.put('   - تم العثور على حل مثالي! إنهاء البحث.')
            break

        if i - last_progress_report >= progress_report_interval:
            unplaced, hard, soft_sum, soft_count = -best_fitness_so_far[0], -best_fitness_so_far[1], -best_fitness_so_far[2], -best_fitness_so_far[3]
            log_q.put(f'--- الدورة {i + 1}/{max_iterations} | أفضل لياقة (ن,ص,م) = ({unplaced}, {hard}, {soft_count}={soft_sum}) ---')
            time.sleep(0.05)
        ...
        if accept_move:
            ...
            if current_fitness > best_fitness_so_far:
                ...
                unplaced, hard, soft_sum, soft_count = -best_fitness_so_far[0], -best_fitness_so_far[1], -best_fitness_so_far[2], -best_fitness_so_far[3]
                log_q.put(f'   >>> إنجاز جديد! أخطاء (نقص, صارم, مرن)=({unplaced}, {hard}, {soft_count}={soft_sum})')
                ...
                send_chart_data(log_q, errors_for_best, use_strict_hierarchy)
    ...
    send_chart_data(log_q, final_failures_list, use_strict_hierarchy)
    time.sleep(0.1)
...
def run_variable_neighborhood_search(
    ...
    unplaced, hard, soft_sum, soft_count = -best_fitness_so_far[0], -best_fitness_so_far[1], -best_fitness_so_far[2], -best_fitness_so_far[3]
    log_q.put(f'   - اكتمل البناء المبدئي. اللياقة (نقص, صارم, مرن) = ({unplaced}, {hard}, {soft_count}={soft_sum})')
    ...
    last_best_fitness = best_fitness_so_far 
    # 🔧 تصحيح وتفعيل حساب حد الركود ديناميكياً لـ VNS
    thresh_val = float(vns_stagnation_threshold)
    actual_threshold = 15.0 if thresh_val >= 100.0 or thresh_val <= 0 else thresh_val
    stagnation_percentage = actual_threshold / 100.0
    ...
    for i in range(max_iterations):
        ...
        if best_fitness_so_far == (0, 0, 0, 0): break
        
        if (i % 10 == 0):
            unplaced, hard, soft_sum, soft_count = -best_fitness_so_far[0], -best_fitness_so_far[1], -best_fitness_so_far[2], -best_fitness_so_far[3]
            log_q.put(f'--- دورة التحسين {i + 1}/{max_iterations} | أفضل لياقة (ن,ص,م) = ({unplaced}, {hard}, {soft_count}={soft_sum}) ---')
            time.sleep(0.01)
           ...
           # --- بداية التعديل: اختيار استراتيجية الهز عشوائياً ---
            ...
            if accept_move:
                ...
                if current_fitness > best_fitness_so_far:
                    ...
                    unplaced_best, hard_best, soft_best_sum, soft_best_count = -best_fitness_so_far[0], -best_fitness_so_far[1], -best_fitness_so_far[2], -best_fitness_so_far[3]
                    log_q.put(f'   >>> إنجاز جديد! أفضل لياقة (ن,ص,م) = ({unplaced_best}, {hard_best}, {soft_best_count}={soft_best_sum})')
...
# =====================================================================
def send_chart_data(log_q, errors_list, use_strict_hierarchy=False):
    """دالة مساعدة لتجميع الأخطاء وإرسالها للمخطط البياني الحي (تمت الترقية للربط الذكي)"""
    import json
    category_counts = {}
    
    # ✨ التحقق مما إذا كان هناك أي خطأ صارم أو نقص (أي خطأ عقوبته 100 أو أكثر)
    has_hard_errors = any(err.get('penalty', 1) >= 100 for err in errors_list)
    
    for err in errors_list:
        penalty = err.get('penalty', 1)
        
        # ✨ الفكرة 1: إذا كان "التسلسل الصارم" مفعلاً، ولا يزال هناك أخطاء صارمة، أخفِ الأخطاء المرنة تماماً!
        if use_strict_hierarchy and has_hard_errors and penalty < 100:
            continue
            
        reason = err.get('reason', '')
        course = err.get('course_name', '')
        
        # تصنيف الخطأ بذكاء
        cat = 'أخطاء أخرى'
        if 'تعارض الأستاذ' in reason: cat = 'تعارض أساتذة'
        elif 'تعارض في القاعة' in reason: cat = 'تعارض قاعات'
        elif 'تعارض قاعة كبيرة' in reason: cat = 'تعارض قاعة كبيرة'
        elif 'الراحة' in reason: cat = 'فترات الراحة'
        elif 'معرفات' in reason: cat = 'تعارض معرفات'
        elif 'المادة لم يتم جدولتها' in reason: cat = 'مواد ناقصة (نقص)'
        elif str(course).startswith('قيد'): cat = course
        else: cat = reason.split()[0] + ' ' + reason.split()[1] if len(reason.split())>1 else reason
        
        # إضافة كلمة (صارم) أو (مرن) ليميزها المستخدم بصرياً
        cat += ' (صارم)' if penalty >= 100 else ' (مرن)'
        
        category_counts[cat] = category_counts.get(cat, 0) + 1

    # ترتيب الأخطاء من الأكبر للأصغر وأخذ أعلى 5 فقط
    sorted_cats = sorted(category_counts.items(), key=lambda x: x[1], reverse=True)[:5]
    
    chart_data = {
        "labels": [x[0] for x in sorted_cats],
        "data": [x[1] for x in sorted_cats]
    }
    log_q.put(f"CHART_DATA:{json.dumps(chart_data)}")
# =====================================================================
...
def calculate_fitness(schedule, all_lectures, days, slots, teachers, rooms_data, levels, identifiers_by_level, special_constraints, teacher_constraints, distribution_rule_type, lectures_by_teacher_map, globally_unavailable_slots, saturday_teachers, teacher_pairs, day_to_idx, rules_grid, last_slot_restrictions, level_specific_large_rooms, specific_small_room_assignments, constraint_severities, 
    ...
    # 2. حساب المواد الناقصة والأخطاء
    ...
    hard_errors_count = 0
    soft_penalty_sum = 0
    soft_errors_count = 0  # ✨ العداد الجديد الذي سيظهر في الشاشة
    filtered_errors_list = []
    
    # تصنيف الأخطاء بناءً على العقوبة الحقيقية وتصفية المعطلة (0 نقطة) تماماً
    for error in errors_list:
        ...
        if penalty >= 100:
            ...
            soft_errors_count += 1  # ✨ زيادة العداد
            filtered_errors_list.append(error)

    # إضافة تفاصيل النقص إلى قائمة الأخطاء المفصلة المصفاة
    for lec in unplaced_lectures:
        filtered_errors_list.append({"course_name": lec.get('name'), "teacher_name": lec.get('teacher_name'), "reason": "المادة لم يتم جدولتها (نقص).", "penalty": 1000})
    ...
    if use_strict_hierarchy:
        ...
        if has_critical_errors:
            # ✨ العمى التام: تصفير المجموع والعدد حتى لا تستخدمها الخوارزمية للمفاضلة إطلاقاً
            fitness_tuple = (-unplaced_count, -hard_errors_count, 0, 0)
        else:
            # عند انتهاء الأخطاء الصارمة، ترفع الخوارزمية العصابة عن عينيها لتبدأ بحل المرن
            fitness_tuple = (0, 0, -soft_penalty_sum, -soft_errors_count)
    else:
        # الوضع الافتراضي (المرن): الخوارزمية ترى وتحسب كل شيء منذ البداية
        fitness_tuple = (-unplaced_count, -hard_errors_count, -soft_penalty_sum, -soft_errors_count)

    return fitness_tuple, filtered_errors_list
# =====================================================================
...
def calculate_schedule_cost(
    ...
    # ✨ الإضافة الجديدة توضع هنا مباشرة ✨
    # إعداد قائمة القاعات المحجوزة مسبقاً لاختبار القفل الشامل
    all_claimed_rooms = set()
    is_global_exclusive = specific_small_room_assignments.get('__GLOBAL_EXCLUSIVE__', False)
    if is_global_exclusive:
        for l, r_vals in specific_small_room_assignments.items():
            if l == '__GLOBAL_EXCLUSIVE__': continue
            if not isinstance(r_vals, list): r_vals = [r_vals]
            for r in r_vals:
                r_actual = str(r)[5:] if str(r).startswith("EXCL_") else str(r)
                all_claimed_rooms.add(r_actual)
    # ✨ نهاية الإضافة ✨
    ...
    for level, day_grid in schedule.items():
        for day_idx, slot_list in enumerate(day_grid):
            for slot_idx, lectures in enumerate(slot_list):
                ...
                # ✨ التعديل 1: منع اجتماع "محاضرة" مع أي مادة أخرى (سواء محاضرة أو تطبيق) في نفس الفترة
                lecture_nature_courses = [lec for lec in lectures if lec.get('course_nature') == 'محاضرة']
                if len(lecture_nature_courses) > 1 or (len(lecture_nature_courses) == 1 and len(lectures) > 1):
                    conflicts_list.append({"course_name": "تعارض بيداغوجي", "teacher_name": level, "reason": f"تعارض محاضرة مع مادة أخرى في {day_name} {slot_name}", "penalty": 100, "involved_lectures": lectures})
                ...
                for lec in lectures:
                    ...
                    if lec.get('room_type') == 'صغيرة':
                        r_vals = specific_small_room_assignments.get(level)
                        actual_rooms = []
                        if r_vals:
                            if not isinstance(r_vals, list): r_vals = [r_vals]
                            actual_rooms = [str(r)[5:] if str(r).startswith("EXCL_") else str(r) for r in r_vals]
                            
                            if lec.get('room') not in actual_rooms:
                                rooms_str = ' أو '.join(actual_rooms)
                                conflicts_list.append({"course_name": lec.get('name'), "reason": f"قيد قاعات الأعمال الموجهة في {day_name} {slot_name}: يجب أن تكون في إحدى القاعات '{rooms_str}' وليس '{lec.get('room')}'", "penalty": 100, "involved_lectures": [lec]})
                        
                        # ✨ معاقبة المستوى إذا دخل قاعة ليست له في حال تفعيل القفل الشامل
                        if is_global_exclusive and lec.get('room') not in actual_rooms:
                            if lec.get('room') in all_claimed_rooms:
                                conflicts_list.append({"course_name": lec.get('name'), "reason": f"تعدي على جناح مشترك: القاعة '{lec.get('room')}' مقفلة حصرياً لمستويات أخرى في {day_name} {slot_name}.", "penalty": 100, "involved_lectures": [lec]})
    ...
    weight_key = constraint_severities.get('consecutive_lectures', 'disabled')
    
    if limit and weight_key != 'disabled':
        penalty_consecutive_lecs = SEVERITY_PENALTIES.get(weight_key, 0)
        if penalty_consecutive_lecs > 0:
            for level, day_grid in schedule.items():
                for day_idx, slot_list in enumerate(day_grid):
                    consecutive_count = 0
                    involved_in_streak = []
                    for slot_idx, lectures in enumerate(slot_list):
                        # نتحقق هل توجد "محاضرة" في هذه الحصة لهذا المستوى؟
                        has_lecture = False
                        current_lectures = []
                        for lec in lectures:
                            # نعتمد على طبيعة المادة أو البصمة النصية
                            if lec.get('course_nature') == 'محاضرة' or '[مح]' in lec.get('name', ''):
                                has_lecture = True
                                current_lectures.append(lec)
                        
                        if has_lecture:
                            consecutive_count += 1
                            involved_in_streak.extend(current_lectures)
                            if consecutive_count > limit:
                                conflicts_list.append({
                                    "course_name": f"تتابع محاضرات ({level})",
                                    "teacher_name": "N/A",
                                    "reason": f"تجاوز الحد الأقصى لتتابع المحاضرات ({limit}) في يوم {days[day_idx]}.",
                                    "penalty": penalty_consecutive_lecs,
                                    "involved_lectures": list(involved_in_streak)
                                })
                        else:
                            # إذا كانت فترة فراغ أو أعمال موجهة، يتم تصفير العداد ليرتاح الطالب
                            consecutive_count = 0
                            involved_in_streak = []
...
def is_placement_valid(lecture, day_idx, slot_idx, final_schedule, teacher_schedule, room_schedule, teacher_constraints, special_constraints, identifiers_by_level, rules_grid, globally_unavailable_slots, rooms_data, saturday_teachers, day_to_idx, level_specific_large_rooms, specific_small_room_assignments, consecutive_large_hall_rule):
    ...
    # ✨ قيد اليوم المقيد (المرن/الصارم)
    global_info = teacher_constraints.get('__GLOBAL__', {})
    restricted_day_name = global_info.get('restricted_day', 'السبت')
    raw_weight = str(global_info.get('weight', 'disabled'))
    
    # خريطة الترجمة لأن البيانات تصل كأرقام ('0', '10', '20') قبل معالجتها
    severity_map = {'20': 'high', '10': 'medium', '1': 'low', '0': 'disabled', 'hard': 'hard'}
    weight_restricted = severity_map.get(raw_weight, raw_weight)
    
    restricted_idx = day_to_idx.get(restricted_day_name, -1)
    
    # يمنع التوزيع فيزيائياً فقط إذا كان المستخدم قد اختار "صارم" (hard)
    if weight_restricted == 'hard' and restricted_idx != -1 and day_idx == restricted_idx:
        if teacher not in saturday_teachers:
            return False, f"الأستاذ غير مسموح له بالعمل في اليوم المقيد ({restricted_day_name})"
...
def validate_teacher_constraints_in_solution(teacher_schedule, special_constraints, teacher_constraints, lectures_by_teacher_map, distribution_rule_type, saturday_teachers, teacher_pairs, day_to_idx, last_slot_restrictions, num_slots, constraint_severities, max_sessions_per_day=None, non_sharing_teacher_pairs=[]):
    ...
    start_end_time_failures = validate_start_end_times(teacher_schedule, special_constraints, teacher_constraints, constraint_severities)
    ...
    # ✨ التحقق المرن من اليوم المقيد (سواء كان السبت أو غيره)
    restricted_day_name = constraint_severities.get('restricted_day', 'السبت')
    restricted_idx = day_to_idx.get(restricted_day_name, -1)
    weight_restricted = constraint_severities.get('saturday_work', 'disabled')

    if restricted_idx != -1 and weight_restricted != 'disabled':
        penalty = SEVERITY_PENALTIES.get(weight_restricted, 0)
        if penalty > 0:
            for teacher_name, slots in teacher_schedule.items():
                if teacher_name not in saturday_teachers:
                    # ✨ التعديل الجوهري: حساب الخطأ لكل "حصة" يدرسها بدلاً من كل "أستاذ"
                    restricted_slots = [s for d, s in slots if d == restricted_idx]
                    for slot_idx in restricted_slots:
                        failures.append({
                            "course_name": f"قيد يوم {restricted_day_name}", 
                            "teacher_name": teacher_name,
                            "reason": f"عمل الأستاذ في اليوم المقيد ({restricted_day_name}) - الفترة {slot_idx + 1}.", 
                            "penalty": penalty,
                            "involved_lectures": lectures_by_teacher_map.get(teacher_name, [])
                        })
...
def validate_start_end_times(teacher_schedule, special_constraints, teacher_constraints, constraint_severities):
    failures = []
    for teacher_name, prof_constraints in special_constraints.items():
        ...
        if prof_manual_days_indices:
            ...
            # ✨ جلب الوزن من الواجهة وتحويله إلى نقاط
            penalty = SEVERITY_PENALTIES.get(constraint_severities.get('start_end_time', 'low'), 1)
            
            min_slot_on_first_day = min(s for d, s in assigned_slots if d == first_day_worked)
            if prof_constraints.get('start_d1_s2') and min_slot_on_first_day < 1:
                failures.append({"course_name": "قيد وقت البدء", "teacher_name": teacher_name, "reason": "بدأ قبل الحصة الثانية في أول يوم عمل له.", "penalty": penalty})
            if prof_constraints.get('start_d1_s3') and min_slot_on_first_day < 2:
                failures.append({"course_name": "قيد وقت البدء", "teacher_name": teacher_name, "reason": "بدأ قبل الحصة الثالثة في أول يوم عمل له.", "penalty": penalty})
            
            max_slot_on_last_day = max(s for d, s in assigned_slots if d == last_day_worked)
            if prof_constraints.get('end_s3') and max_slot_on_last_day > 2:
                failures.append({"course_name": "قيد وقت الإنهاء", "teacher_name": teacher_name, "reason": "انتهى بعد الحصة الثالثة في آخر يوم عمل له.", "penalty": penalty})
            if prof_constraints.get('end_s4') and max_slot_on_last_day > 3:
                failures.append({"course_name": "قيد وقت الإنهاء", "teacher_name": teacher_name, "reason": "انتهى بعد الحصة الرابعة في آخر يوم عمل له.", "penalty": penalty})
...
# =====================================================================
# START: VNS LOCAL SEARCH (IMPROVED WITH 70/30 RANDOM EXPLORATION)
# =====================================================================
def run_vns_local_search1(
    ...
    """
    بحث محلي هجين ذكي (70% موجه للإصلاح / 30% استكشاف عشوائي).
    يمنع الخوارزمية من الوقوع في فخ الحلول المحلية من خلال تحريك مواد سليمة لإفساح المجال.
    """
    ...
    for _ in range(max_iterations):
        ...
        # ✨ --- التعديل الجذري: تطبيق قاعدة الاستكشاف العشوائي (30%) --- ✨
        exploration_chance = random.random()
        
        if exploration_chance < 0.30 or not (hard_errors or soft_errors):
            move_type = 'explore' # 30% فرصة لهز مواد سليمة لإفساح مساحات
        else:
            move_type = 'repair'  # 70% إصلاح موجه للأخطاء
            if hard_errors and random.random() < 0.8:
                target_error = random.choice(hard_errors)
            else:
                target_error = random.choice(soft_errors or hard_errors)
        ...
        if move_type == 'repair':
            ...
            # الاستكشاف العشوائي: نختار أستاذاً عشوائياً تماماً ونعيد توزيع مواده لخلط الأوراق
            random_teacher = random.choice(teachers)['name']
            lectures_to_reinsert = [lec for lec in all_lectures if lec.get('teacher_name') == random_teacher]
            if not lectures_to_reinsert: continue
            ...
            # في الاستكشاف لا نقوم بترتيب المحاضرات، بل ندرجها بترتيب عشوائي لضمان التغيير الجذري
            random.shuffle(lectures_to_reinsert)
            for lecture in lectures_to_reinsert:
                find_slot_for_single_lecture(lecture, temp_schedule, temp_teacher_schedule, temp_room_schedule, days, slots, rules_grid, rooms_data, teacher_constraints, globally_unavailable_slots, special_constraints, primary_slots, reserve_slots, identifiers_by_level, prioritize_primary, saturday_teachers, day_to_idx, level_specific_large_rooms, specific_small_room_assignments, consecutive_large_hall_rule, prefer_morning_slots)
...
def find_slot_for_single_lecture(lecture, final_schedule, teacher_schedule, room_schedule,
    ...
    best_placement = None
    # إرجاع الحرية: توجيه أولوية الفترات الصباحية للمدرجات فقط، أما المحاضرات في القاعات فتوزع بمرونة
    ...
    if best_placement:
        ...
        details = {
            ...
            "course_nature": lecture.get('course_nature'), # ✨ نقل الهوية البيداغوجية إلى الجدول
...
def _find_best_greedy_placement_in_slots(slots_to_search, lecture, final_schedule, teacher_schedule, teacher_constraints, special_constraints, identifiers_by_level, rules_grid, globally_unavailable_slots, room_schedule, rooms_data, saturday_teachers, day_to_idx, level_specific_large_rooms, specific_small_room_assignments, consecutive_large_hall_rule, prefer_morning_slots=False):
    ...
    for day_idx, slot_idx in slots_to_search:
        ...
        # ✨✨ الإضافة الجديدة لكي تتهرب الخوارزمية الطماعة من اليوم المقيد إذا كان مرناً ✨✨
        global_info = teacher_constraints.get('__GLOBAL__', {})
        restricted_day_name = global_info.get('restricted_day', 'السبت')
        raw_weight = str(global_info.get('weight', 'disabled'))
        
        # خريطة الترجمة لضمان تعرف الخوارزمية على حالة "معطل"
        severity_map = {'20': 'high', '10': 'medium', '1': 'low', '0': 'disabled', 'hard': 'hard'}
        weight_restricted = severity_map.get(raw_weight, raw_weight)
        
        restricted_idx = day_to_idx.get(restricted_day_name, -1)

        if weight_restricted != 'disabled' and restricted_idx != -1 and day_idx == restricted_idx:
            if lecture.get('teacher_name') not in saturday_teachers:
                # خصم نقاط قاسية حتى يتهرب الخوارزمي الطماع من هذا اليوم بقوة
                penalty_val = SEVERITY_PENALTIES.get(weight_restricted, 10)
                current_fitness -= (penalty_val * 10)
...
def _find_valid_and_available_room(lecture, day_idx, slot_idx, final_schedule, room_schedule, rooms_data, rules_grid, identifiers_by_level, level_specific_large_rooms, specific_small_room_assignments):
    ...
    exclusive_rooms_for_other_levels = set()
    allowed_specific_rooms_per_level = []
    allowed_types_per_level_list = []
    
    # قراءة حالة القفل الشامل
    is_global_exclusive = specific_small_room_assignments.get('__GLOBAL_EXCLUSIVE__', False)
    all_claimed_rooms = set()
    lecture_claimed_rooms = set()
    ...
    for l, r_vals in specific_small_room_assignments.items():
        ...
        level_rooms = set()
        for r_val in r_vals:
            is_excl = str(r_val).startswith('EXCL_')
            r_actual = str(r_val)[5:] if is_excl else str(r_val)
            
            all_claimed_rooms.add(r_actual) # تسجيل القاعة كممتلكات محجوزة
            
            if l in levels_for_lecture and lecture_room_type_needed == 'صغيرة':
                level_rooms.add(r_actual)
                lecture_claimed_rooms.add(r_actual)
            elif l not in levels_for_lecture and is_excl:
                exclusive_rooms_for_other_levels.add(r_actual) # طرد بسبب القفل الفردي
                
        if l in levels_for_lecture and level_rooms:
            allowed_specific_rooms_per_level.append(level_rooms)

    # ✨ تطبيق القفل الشامل للأجنحة (منع الغرباء من الدخول)
    if is_global_exclusive and lecture_room_type_needed == 'صغيرة':
        for r in all_claimed_rooms:
            # إذا كانت القاعة مملوكة، ولكن ليست ملكاً لهذا المستوى، اطرد المستوى منها!
            if r not in lecture_claimed_rooms:
                exclusive_rooms_for_other_levels.add(r)

    for level in levels_for_lecture:
        lectures_in_slot = final_schedule[level][day_idx][slot_idx]
        is_current_lecture_primary = lecture.get('course_nature') == 'محاضرة'
        has_primary_in_slot = any(l.get('course_nature') == 'محاضرة' for l in lectures_in_slot)
        
        if lectures_in_slot and (is_current_lecture_primary or has_primary_in_slot): return None 
        ...
        if current_identifier:
            ...
            if current_identifier in used_identifiers: return None 

        if lecture_room_type_needed == 'كبيرة':
            if room := level_specific_large_rooms.get(level):
                allowed_specific_rooms_per_level.append({room})

        rules_for_slot = rules_grid[day_idx][slot_idx]
        level_specific_rules = [r for r in rules_for_slot if level in r.get('levels', [])]
        if any(r.get('rule_type') == 'NO_HALLS_ALLOWED' for r in level_specific_rules): return None 
        ...
        if not level_specific_rules:
            ...
            for rule in level_specific_rules:
                ...
                if rule_type == 'ANY_HALL': current_level_allowed_types.update(['كبيرة', 'صغيرة'])
                    ...
                    if hall := rule.get('hall_name'): allowed_specific_rooms_per_level.append({hall})
            allowed_types_per_level_list.append(current_level_allowed_types)

    final_required_rooms = None
    if allowed_specific_rooms_per_level:
        final_required_rooms = set.intersection(*allowed_specific_rooms_per_level)
        if not final_required_rooms: return None 

    final_allowed_types = set.intersection(*allowed_types_per_level_list) if allowed_types_per_level_list else set()
    if lecture_room_type_needed not in final_allowed_types: return None 

    filtered_rooms_data = []
    for r in rooms_data:
        r_name = r.get('name')
        if r_name in exclusive_rooms_for_other_levels: continue
        if final_required_rooms is not None and r_name not in final_required_rooms: continue
        filtered_rooms_data.append(r)

    return find_available_room(day_idx, slot_idx, room_schedule, filtered_rooms_data, [lecture_room_type_needed], None)
...
def calculate_lecture_difficulty(lecture, all_lectures_for_teacher, special_constraints, manual_days):
    ...
    # إرجاع الحرية: الأولوية القصوى في التوزيع للموارد النادرة (المدرجات)
        ...
        for item in sorted(candidate_lectures, key=lambda x: x['original_slot'], reverse=True):
            ...
            if refinement_level == 'deep':
                ...
                if refinement_level == 'simple_restricted':
                    teacher_work_days = [original_day] # البحث في نفس اليوم فقط!
                else:
                ...
                for target_day_idx in teacher_work_days:
                    for target_slot_idx in range(original_slot):
                        ...
                        if accept_move:
                            ...
                            send_chart_data(log_q, new_violations_failures, False)

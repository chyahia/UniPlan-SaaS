...
def execute_resit_distribution(tenant_id, algo_choice, duration, destruction_rate, strategy, celery_task=None):
    ...
    # ✨ دالة فحص الإيقاف: تقرأ الإشارة من الذاكرة حسب نوع البيئة
    def check_stop():
        from flask import current_app
        mode = current_app.config.get('APP_MODE', 'desktop')
        if mode == 'desktop':
            from app.memory_logger import store
            with store.lock:
                return store.status.get(f"stop_{tenant_id}", False)
        else:
            from app.redis_logger import redis_client
            return redis_client.get(f"stop_{tenant_id}") is not None
    ...
    best_dist, violations = run_distribution(
        ...
        check_stop=check_stop  # ✨ تمرير دالة الإيقاف هنا
    )

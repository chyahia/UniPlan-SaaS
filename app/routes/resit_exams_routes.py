...
def start_solver():
    ...
    if mode == 'desktop':
        ...
        with store.lock:
            ...
            store.status[f"stop_{tenant_id}"] = False # ✨ إضافة تصفير إشارة الإيقاف
        ...
        redis_client.delete(f"stop_{tenant_id}") # ✨ إضافة تصفير إشارة الإيقاف
...
@resit_exams_bp.route('/stop_solver', methods=['POST'])
def stop_solver():
    tenant_id = session.get('tenant_id')
    from flask import current_app
    mode = current_app.config.get('APP_MODE')
    ...
    if mode == 'desktop':
        from app.memory_logger import store
        with store.lock:
            store.status[f"stop_{tenant_id}"] = True
    else:
        from app.redis_logger import redis_client
        redis_client.set(f"stop_{tenant_id}", "1", ex=3600)

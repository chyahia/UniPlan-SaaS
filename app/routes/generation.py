...
def import_excel():
    ...
    try:
        ...
        # ✨ السطر الجديد: جلب جميع القاعات من قاعدة البيانات بطريقة SQLAlchemy الآمنة للسحابة
        tenant_id = session.get('tenant_id')
        all_rooms = [r.name for r in Room.query.filter_by(tenant_id=tenant_id).all()]

        # ✨ السطر الجديد: حساب القاعات الفارغة المحدثة للجدول الشبكي
        ...
        return jsonify({
            ...
            "free_rooms": free_rooms  # ✨ إرسالها الآن للواجهة
        })
...
def import_comprehensive_excel():
    ...
    file = request.files['file']
    tenant_id = session.get('tenant_id') # ✨ جلب معرف القسم من الجلسة
    
    # محاولة قراءة الأيام والفترات من الواجهة
    days = json.loads(request.form.get('days', '[]'))
    slots = json.loads(request.form.get('slots', '[]'))
    
    # ✨ الاتصال بالقاعدة باستخدام SQLAlchemy (متوافق مع السحابة)
    if not days or not slots:
        struct_setting = Setting.query.filter_by(key='schedule_structure', tenant_id=tenant_id).first()
        if struct_setting and struct_setting.value:
            structure_data = json.loads(struct_setting.value)
    ...
    # ✨ جلب قائمة جميع القاعات باستخدام نماذج SQLAlchemy
    all_rooms = [r.name for r in Room.query.filter_by(tenant_id=tenant_id).all()]
    ...
    try:
        ...
        # ✨ 1. إضافة قاموس الترجمة العكسي للأيام
        reverse_days_map = {
            # الإنجليزية
            'sunday': 'الأحد', 'monday': 'الإثنين', 'tuesday': 'الثلاثاء',
            'wednesday': 'الأربعاء', 'thursday': 'الخميس', 'friday': 'الجمعة', 'saturday': 'السبت',
            # الفرنسية
            'dimanche': 'الأحد', 'lundi': 'الإثنين', 'mardi': 'الثلاثاء',
        ...
        for sheet_name in wb.sheetnames:
            ...
            # تجاوز الصف الأول (رؤوس الأعمدة) والبدء من الصف الثاني
            for row_idx in range(2, ws.max_row + 1):
                day_name = str(ws.cell(row=row_idx, column=1).value).strip()      
                ...
                # ✨ 2. استخدام القاموس العكسي لإرجاع اليوم لأصله العربي
                day_name_lower = day_name.lower()
                if day_name_lower in reverse_days_map:
                    day_name = reverse_days_map[day_name_lower]
                    
                # الآن هذا الشرط سيعمل بكفاءة تامة مهما كانت لغة الملف!
                if day_name in days and slot_name in slots:
                    ...
                    if course_type_lower in ["محاضرة", "lecture", "cours"]: 
                        nature_tag = "[مح]"
                    elif course_type_lower in ["أعمال تطبيقية", "tp"]: 
                        nature_tag = "[أت]"
                    
                    full_course_name = f"{course_name} {nature_tag}"
                    
                    if teacher_name.lower() in ["none", "بدون أستاذ", "-", "no teacher", "sans professeur"]: 
                        teacher_name = ""
                    if room_name.lower() in ["none", "بدون قاعة", "-", "no room", "sans salle"]: 
                        room_name = ""
        ...
        # ✨ حساب القاعات الفارغة المحدثة بناءً على الجدول الجديد
        free_rooms = [[ [] for _ in slots ] for _ in days]
        for d_idx in range(len(days)):
            for s_idx in range(len(slots)):
                busy_rooms = set()
                # جمع القاعات المشغولة في هذه الفترة من جميع المستويات
                for grid_level in new_schedule.values():
                    if d_idx < len(grid_level) and s_idx < len(grid_level[d_idx]):
                        for lec in grid_level[d_idx][s_idx]:
                            rm = lec.get('room')
                ...
                # طرح القاعات المشغولة من إجمالي القاعات للحصول على القاعات الفارغة
                for r_name in all_rooms:
                    if r_name.strip() not in busy_rooms:
                        free_rooms[d_idx][s_idx].append(r_name)
        ...
        return jsonify({
            ...
            "schedule": new_schedule, 
            "prof_schedules": new_prof_schedules,
            "free_rooms": free_rooms # ✨ إرسال القاعات الفارغة للواجهة
        })
        
    except Exception as e:
        import traceback
        traceback.print_exc()
        return jsonify({"error": "فشل في قراءة ملف الإكسل الشامل. التفاصيل: " + str(e)}), 500